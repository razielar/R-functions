...
f.lncRNA.mRNA.pair <- function(expression_matrix, lncRNA_mRNA){
  ...
  for(i in 1:nrow(lncRNA_mRNA)){
      ...
      print(paste("lncRNA: ",gene_pairs[1], "mRNA: ", gene_pairs[2]))
      ...
      if(nrow(df_plot) == 0){
          ...
          rlang::warn(message = "Both lncRNA and mRNA are not present",
                      .subclass = "both missing")
          
      } else if(nrow(df_plot) == 1){
          
          rlang::warn(message = "The Dataframe should have two values",
                      .subclass = "one missing value")
          
          df_plot <- melt(t(df_plot))
          
          df_plot$Gene <- rep("Something" , nrow(df_plot)) #IMPROVE THIS!!!!
          df_list[[i]] <- df_plot
